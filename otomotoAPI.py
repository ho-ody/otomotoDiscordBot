...
def set_option_refresh_ignore(setto):
    global OPTION_REFRESH_IGNORE
    OPTION_REFRESH_IGNORE = setto
def get_option_refresh_ignore():
    return OPTION_REFRESH_IGNORE
async def test():
...
def checkIfDateShouldBeCount(date: str):
    ...
    minutes = int(minutes)
    if minutes > OPTION_REFRESH_IGNORE:
        return False
    return True
